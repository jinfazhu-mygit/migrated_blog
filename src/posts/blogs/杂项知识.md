...
const downloadFile = async (url: string, filename: string): Promise<void> => {
  try {
    console.log("%c Line:411 🍺 url", "color:#e41a6a", url, filename);
    ...
    console.error('使用fetch下载失败，回退到直接下载:', error);
    // 降级方案：使用原始的直接下载方式
...
// 保留原有的直接下载方式作为降级方案
const directDownload = (url: string, filename: string): void => {
  const a = document.createElement('a');
  a.href = url;
  a.download = filename || 'download';
  a.style.display = 'none';
...
### Blob文件流获取，文件流转File上传(同<Upload />组件)，获取文件链接
...
```jsx
  const downloadWaterMarkFile = async (data: any) => {
    try {
      ...
      console.log('%c Line:39 🌽 res', 'color:#ea7e5c', res, res instanceof Blob);
    } catch (error) {
      // console.log('%c Line:60 🥔 error', 'color:#42b983', error);
      ...
      if (error instanceof Blob) {
        ...
        const fileName = `隐患报送_${dayjs().format('YYYYMMDDHHmmss')}.docx`;

        // 2.文件流Blob转File进行上传，获取文件链接
        ...
        try {
          ...
          console.log('%c Line:86 🍑 res，文件后缀', 'color:#93c0a4', res, filePath, '请求', path);
          Toast.show({ content: `res，文件后缀${path}`, duration: 3000 });

          // 3.发送action获取链接触发手机下载
          const action = {
            actionType: 'download_action',
            extra: {
              path: path,
              // fileName总长度不能超过82否则app下载文件会出错 fileName90+ _xxx四位时间戳 + .docx五位后缀名
...
### window.location.href和window.open

**window.location.href适用于改变当前页面的地址路径**

**window.open适用于保留当前页面，新开一个tab页(不适合用于在app内嵌套的h5内使用)**
...
//解析URL
export function parseUrl(str: any): any {
...
if (titleIcon !== null) {
  titleIcon.href = decodeURIComponent(require('./images/123333.png'))
}
// 动态修改浏览器title
...
```js
<div
  dangerouslySetInnerHTML={{ __html: this.state.psychologyInformationItem.content }}
  className="lg_rich_text_show"
></div>
```
...
window.addEventListener(
  ...
  false,
) // 第三个参数为false表示该元素在事件的“冒泡阶段”（由内向外传递时）响应事件。
```
...
### 时间戳转'刚刚'、'1分钟前'、'1小时前'、'2025-08-19 06:50:50'转为'刚刚'、'1分钟前'、'1小时前'
...
// 日期时间转时间戳
Date.parse('2024-01-24 15:30:00')(
  // 1706081400000
  // 获取当前星期数值 1234567
  new Date(),
).getDay()
...
```ts
export function _debounce(fn: any, delay: number, immediate = false, resCallback?: any) {
  let timer: any = null
  let isInvoke = false
...
export function _throttle(fn: any, interval: number, options = { leading: true, trailing: false }) {
  // 默认第一次触发，最后一次不触发
  let lastTime = 0
...
export class IndexMobx {
  ...
  getPageList = async (data: any) => {
    ...
    console.log('%c Line:98 🌽 列表数据', 'color:#6ec1c2', resData)
...
```css
touch-action: auto;
touch-action: none; // 可能会限制其他时间如滚动、点击，需具体测试后使用
touch-action: pan-y; // 限制横向缩放，不限制竖向滚动、点击事件
```
...
decodeURIComponent(base64.encode(url))
```

## 文件上传(antd, antd-mobile, Taro.chooseMedia(wx.chooseMedia)方式)
  ...
  const uploadClick = () => {
    Taro.chooseMedia({
      ...
      success: (res) => {
        ...
        Taro.uploadFile({
          url: BASE_URL + `/blade-resource/oss/endpoint/put-file`, //仅为示例，非真实的接口地址
          // url: apiConfig.baseUrl + `/blade-resource/oss/endpoint/put-file`, //仅为示例，非真实的接口地址
          ...
          success(rea) {
            ...
            console.log('%c Line:65 🥟 rea', 'color:#7f2b82', rea, data)
            if (data.code === 200 && data.success) {
              ...
              Taro.showToast({ title: `文件上传失败 ${data.msg}`, icon: 'none' })
            }
          },
          fail: (err) => {
            Taro.hideLoading()
            Taro.showToast({ title: `文件上传失败 ${err}`, icon: 'none' })
          },
        })
      },
      fail: (err) => {
        // Taro.showToast({ title: `文件选择失败 ${err}`, icon: 'none' })
      },
    })
  }
}
...
const Index: FC<IndexProps> = (props) => {
  ...
  const formatTime = (seconds: any) => {
    console.log("%c Line:102 🥐 seconds", "color:#b03734", seconds);
    ...
    <div className={styles['sound_record']}>
      <div className={styles['record_radius']}>
        <div className={`${styles['center_radius']} ${vm.recordState == 1 ? styles['center_radius_active'] : ''}`} onClick={radiusClick}>
          ...
          <div className={styles['state_text']}>{vm.recordState == 0 ? '开始录音' : vm.recordState == 1 ? '录音中' : vm.recordState == 2 ? '继续录音' : vm.recordState == 3 ? '完成录音' : ''}</div>
        </div>
      </div>
...
## js-audio-recorder在react中实现录音功能
...
export class IndexMobx {
  ...
  uploadRecordingFile = async () => {
    ...
    formData.append('fileType', '3') // 1导入音频 2导入文本 3实时录制
    const res: any = await request({
    ...
    console.log('%c Line:39 🍉 转写res', 'color:#93c0a4', res)
  }
...
const Index: FC<IndexProps> = (props) => {
  ...
  const backClick = () => {
    if (vm.recordState == 0) {
      ...
      Toast.show({ content: '正在录音中，无法返回' })
    } else if (vm.recordState == 2) {
      Toast.show({ content: '录音未结束，无法返回' })
    } else if (vm.recordState == 3) {
      Dialog.show({
        content: '您录制的会议还未进行转写，不支持保存到本地，确认需要退出此会议信息吗？',
        closeOnAction: true,
        actions: [[
          {
            key: 'cancel',
  ...
  useEffect(() => {
    if (duration >= 6 * 60 * 60 - 1) {
      Toast.show({ content: '已达到最长录音时长6小时，自动结束录音' })
      stopRecording()
    }
  }, [duration]);
  ...
  const stopRecording = () => {
    ...
    console.log("%c Line:142 🍪 audioFile", "color:#ffdd4d", blob, audioFile);
  ...
  const cancelRecordClick = () => {
    ...
    if ([1, 2].includes(vm.recordState)) {
      Toast.show({ content: '录音操作还未结束，无法取消' });
      return
    } else if (vm.recordState == 3) {
      if (vm.audioUrl) {
        if (vm.exChangeState == 1) {
          Toast.show({ content: '录音转写中，无法取消' });
          return
        } else if (vm.exChangeState == 0 || vm.exChangeState == 2 || vm.exChangeState == 3) {
          Dialog.show({
            content: vm.exChangeState == 0 ? '您录制的会议还未进行转写，不支持保存到本地，确认需要删除此会议信息吗？'
              : `您录制的会议转写${vm.exChangeState == 2 ? '成功' : '失败'}，确认重置状态开始新的录制吗？`,
            closeOnAction: true,
            actions: [[
              {
                key: 'cancel',
    ...
    <>
      <div className={styles['sound_record']}>
        ...
        <div className={styles['bottom_three']}>
          <div className={`${styles['left']} ${[1, 2, 3].includes(vm.recordState) ? styles['left_active'] : ''}`} onClick={cancelRecordClick}>取消</div>
          ...
          <div className={`${styles['right']} ${vm.recordState == 3 ? styles['right_avtice'] : ''}`} onClick={exChangeRecording}>开始转写</div>
        </div>
      </div>
    </>
  )
...
## floating-ui自定义tooltip

[floating-ui官网](https://floating.nodejs.cn/docs/react-examples)
...
export const MouseFollower: React.FC<MouseFollowerProps> = ({
  ...
  // 使用 Floating UI 的定位系统
  const { refs, floatingStyles, context } = useFloating({
    open: isOpen,
    onOpenChange: setIsOpen,
    middleware: [
      offset(10), // 距离触发元素的偏移量
      shift({ padding: 5 }), // 防止超出屏幕
    ],
    placement: 'top-start',
    whileElementsMounted: autoUpdate,
    strategy: followMouse ? 'fixed' : 'absolute',
  });

  // 鼠标事件处理
  const hover = useHover(context, { move: followMouse });
        ...
        {isOpen && (
          <div
            ...
            style={{
              ...floatingStyles,
              zIndex: 9999,
              background: 'rgba(0, 76, 172, 0.90)',
              padding: '8px 12px',
              borderRadius: '6px',
              boxShadow: '0 2px 8px rgba(0,0,0,0.15)',
              border: '1px solid rgba(32, 121, 229, 1)',
              maxWidth: '300px',
              pointerEvents: 'none', // 防止浮动层干扰鼠标事件
            }}
            {...getFloatingProps()}
          >
            {content}
...
## 电视端大屏(焦点管理、键盘事件、遥控控制)

通过键盘上下左右、enter键位控制焦点在界面的移动，onclick触发事件

![pZoNEqS.png](https://s41.ax1x.com/2026/02/06/pZoNEqS.png)
...
interface FocusPosition {
  area: 'close' | 'filter_row' | 'confirm';
  row: number;
  col: number;
}

const ModalContent: FC<IndexProps> = (props: any) => {
  const { closeDrawer, confirmDrawer } = props;
  const vm = useLocalObservable(() => IndexMobx);

  // 焦点管理
  const [focusPosition, setFocusPosition] = useState<FocusPosition>({
    area: 'filter_row', // close filter_row confirm
    row: 0,
    col: 0
  });
  ...
  // 存储每个select_item的引用，用于滚动到头部
  const selectItemRefs = useRef<(HTMLDivElement | null)[]>([]);

  // 构建筛选行的数据结构
  const filterRows = [
    {
      title: '学段',
      options: vm.eduPhaseList,
      selected: vm.eduPhase,
      onSelect: (code: string) => vm.setInfo({ eduPhase: code }),
      colCount: vm.eduPhaseList.length
    },
    {
      title: '年级',
      options: vm.eduGradeList,
      selected: vm.eduGrade,
      onSelect: (code: string) => vm.setInfo({ eduGrade: vm.eduGrade === code ? '' : code }),
      colCount: vm.eduGradeList.length
    },
    {
      title: '出版社',
      options: vm.bookPublisherList,
      selected: vm.bookPublisher,
      onSelect: (code: string) => vm.setInfo({ bookPublisher: vm.bookPublisher === code ? '' : code }),
      colCount: vm.bookPublisherList.length
    },
    {
      title: '册别',
      options: vm.bookVolumeList,
      selected: vm.bookVolume,
      onSelect: (code: string) => vm.setInfo({ bookVolume: vm.bookVolume === code ? '' : code }),
      colCount: vm.bookVolumeList.length
    }
  ];

  // 滚动到焦点元素的函数
  const scrollToFocusedElement = (element: HTMLElement | null, area: string, row: number) => {
    if (!element || !scrollContainerRef.current) return;

    const container = scrollContainerRef.current;

    // 如果是第一行（学段），滚动到头部
    if (area === 'close' || area === 'filter_row' && row === 0) {
      // 直接滚动到容器顶部
      container.scrollTo({
        top: 0,
        behavior: 'smooth'
    ...
    if (!isInView) {
      ...
      if (elementRect.top < containerRect.top) {
        // 滚动到元素顶部对齐容器顶部
        container.scrollTo({
          top: elementTopRelativeToContainer - 20, // 留一点边距
          behavior: 'smooth'
        });
      }
      // 如果元素在容器下方
        ...
        // 滚动到元素底部对齐容器底部
        container.scrollTo({
          top: elementTopRelativeToContainer - containerHeight + elementHeight + 20, // 留一点边距
          behavior: 'smooth'
        });
      }
    }
  ...
  // 焦点移动后自动滚动到焦点元素
  useEffect(() => {
    const { area, row, col } = focusPosition;
    console.log("%c Line:133 🍆 area, row, col", "color:#f5ce50", area, row, col);
    let element: HTMLElement | null = null;

    if (area === 'close') {
      element = focusableElementsRef.current.close[0];
      // 如果是关闭按钮，滚动到顶部
      ...
      // element = focusableElementsRef.current.confirm[0];
    } else if (area === 'filter_row') {
      const rowElements = focusableElementsRef.current.filter_row[row];
      if (rowElements && rowElements[col]) {
        element = rowElements[col];
      }
    }
    ...
    if (element) {
      ...
      requestAnimationFrame(() => {
        scrollToFocusedElement(element, area, row);
      });
    }
  }, [focusPosition]);
  ...
  const handleMoveUp = () => {
    ...
    if (area === 'close') {
      ...
      if (row === 0) {
        ...
        // 向上移动到上一行筛选条件
        const prevRow = row - 1;
        const prevRowOptions = filterRows[prevRow];
        const targetCol = Math.min(col, prevRowOptions.options.length - 1);
        setFocusPosition({
          area: 'filter_row',
          row: prevRow,
          col: targetCol
      ...
      // 从确定按钮移动到第四行筛选条件
      const targetRow = 3; // 第四行
      const targetCol = Math.min(col, filterRows[targetRow].options.length - 1);
      setFocusPosition({
        area: 'filter_row',
        row: targetRow,
        col: targetCol
  ...
  const handleMoveDown = () => {
    ...
    if (area === 'close') {
      // 从关闭按钮移动到第一行筛选条件
      const targetCol = Math.min(col, filterRows[0].options.length - 1);
      setFocusPosition({
        area: 'filter_row',
        row: 0,
        col: targetCol
      ...
      if (row === 3) {
        ...
        // 向下移动到下一行筛选条件
        const nextRow = row + 1;
        const nextRowOptions = filterRows[nextRow];
        const targetCol = Math.min(col, nextRowOptions.options.length - 1);
        setFocusPosition({
          area: 'filter_row',
          row: nextRow,
          col: targetCol
  ...
  const handleMoveLeft = () => {
    ...
    if (area === 'close') {
      // 关闭按钮向左移动（如果有多个关闭按钮，这里只有一个）
      // 可以移动到最后一行的最后一个筛选条件
      const lastRow = 3;
      const lastRowOptions = filterRows[lastRow];
      const targetCol = lastRowOptions.options.length - 1;
      setFocusPosition({
        area: 'filter_row',
        row: lastRow,
        col: targetCol
      ...
      if (col > 0) {
        ...
        const prevRow = row - 1;
        const prevRowOptions = filterRows[prevRow];
        const targetCol = prevRowOptions.options.length - 1;
        setFocusPosition({
          area: 'filter_row',
          row: prevRow,
          col: targetCol
  ...
  const handleMoveRight = () => {
    ...
    if (area === 'close') {
      ...
      const currentRowOptions = filterRows[row];
      if (col < currentRowOptions.options.length - 1) {
        setFocusPosition({
          area: 'filter_row',
          row: row,
          col: col + 1
  ...
  const handleConfirm = () => {
    ...
    if (area === 'close') {
      ...
      const currentRow = filterRows[row];
      if (currentRow && currentRow.options[col]) {
        const option = currentRow.options[col];
        currentRow.onSelect(option.code);
      }
    } else if (area === 'confirm') {
      filterConfirm();
    }
  ...
  useEffect(() => {
    // 根据学段设置年级列表、学科列表
    vm.eduPhaseRelateFilter(vm.eduPhase, true, false);
  ...
  // 渲染焦点样式
  const renderFocusStyle = (area: string, row?: number, col?: number) => {
    const { area: focusArea, row: focusRow, col: focusCol } = focusPosition;

    if (area === focusArea) {
      if (area === 'close' || area === 'confirm') {
        // 对于单元素区域，只需要检查area
        return styles.focused;
      } else if (area === 'filter_row') {
        // 对于多行多列区域，检查行和列
        return focusRow === row && focusCol === col ? styles.focused : '';
      }
    }
    return '';
  };
  ...
  return (
    <div className={styles['custom_drawer_content']}>
      {/* 关闭按钮 */}
      <div
        className={`${styles['right_close']} ${renderFocusStyle('close')}`}
        onClick={closeDrawerFn}
        ref={el => {
          if (el) {
            focusableElementsRef.current.close[0] = el;
          }
        }}
      ></div>
      ...
      >
        {/* 学段 */}
        <div
          className={`${styles['select_item']} ${styles['first']}`}
          ref={el => {
            selectItemRefs.current[0] = el;
          }}
        >
          <div className={styles['text']}>学段</div>
          <div className={styles['select_list']}>
            {vm.eduPhaseList.map((item: any, index: number) => (
              <div
                key={index}
                className={`${vm.eduPhase === item.code ? styles['active_item'] : ''} ${styles['select_tag']} ${renderFocusStyle('filter_row', 0, index)}`}
                onClick={() => { vm.setInfo({ eduPhase: item.code }) }}
                ref={el => {
                  if (el) {
                    if (!focusableElementsRef.current.filter_row[0]) {
                      focusableElementsRef.current.filter_row[0] = [];
                    }
                    focusableElementsRef.current.filter_row[0][index] = el;
                  }
                }}
              >
                {item.name}
        ...
        {/* 年级 */}
        <div
          className={`${styles['select_item']} ${styles['second']}`}
          ref={el => {
            selectItemRefs.current[1] = el;
          }}
        >
          <div className={styles['text']}>年级</div>
          <div className={styles['select_list']}>
            {vm.eduGradeList.map((item: any, index: number) => (
              <div
                key={index}
                className={`${vm.eduGrade === item.code ? styles['active_item'] : ''} ${styles['select_tag']} ${renderFocusStyle('filter_row', 1, index)}`}
                onClick={() => { vm.setInfo({ eduGrade: vm.eduGrade === item.code ? '' : item.code }) }}
                ref={el => {
                  if (el) {
                    if (!focusableElementsRef.current.filter_row[1]) {
                      focusableElementsRef.current.filter_row[1] = [];
                    }
                    focusableElementsRef.current.filter_row[1][index] = el;
                  }
                }}
              >
                {item.name}
        ...
        {/* 出版社 */}
        <div
          className={`${styles['select_item']} ${styles['third']}`}
          ref={el => {
            selectItemRefs.current[2] = el;
          }}
        >
          <div className={styles['text']}>出版社</div>
          <div className={styles['select_list']}>
            {vm.bookPublisherList.map((item: any, index: number) => (
              <div
                key={index}
                className={`${vm.bookPublisher === item.code ? styles['active_item'] : ''} ${styles['select_tag']} ${renderFocusStyle('filter_row', 2, index)}`}
                onClick={() => { vm.setInfo({ bookPublisher: vm.bookPublisher === item.code ? '' : item.code }) }}
                ref={el => {
                  if (el) {
                    if (!focusableElementsRef.current.filter_row[2]) {
                      focusableElementsRef.current.filter_row[2] = [];
                    }
                    focusableElementsRef.current.filter_row[2][index] = el;
                  }
                }}
              >
                {item.name}
        ...
        <div
          className={`${styles['select_item']} ${styles['third']}`}
          ref={el => {
            selectItemRefs.current[3] = el;
          }}
        >
          <div className={styles['text']}>册别</div>
          <div className={styles['select_list']}>
            {vm.bookVolumeList.map((item: any, index: number) => (
              <div
                key={index}
                className={`${vm.bookVolume === item.code ? styles['active_item'] : ''} ${styles['select_tag']} ${renderFocusStyle('filter_row', 3, index)}`}
                onClick={() => { vm.setInfo({ bookVolume: vm.bookVolume === item.code ? '' : item.code }) }}
                ref={el => {
                  if (el) {
                    if (!focusableElementsRef.current.filter_row[3]) {
                      focusableElementsRef.current.filter_row[3] = [];
                    }
                    focusableElementsRef.current.filter_row[3][index] = el;
                  }
                }}
              >
                {item.name}
      ...
      <div className={styles['bottom_btn']}>
        <div
          className={`${styles['confirm_btn']} ${renderFocusStyle('confirm')}`}
          onClick={filterConfirm}
          ref={el => {
            if (el) {
              focusableElementsRef.current.confirm[0] = el;
            }
          }}
        >
          确定
...
.custom_drawer_content {
  width: 100%;
  height: 100%;
  padding: vh(15) vw(10) vh(16) vw(20);
  position: relative;
  display: flex;
  ...
  .right_close {
    ...
    top: vh(15);
    right: vw(20);
    background: url('@/images/tvScreenView/modal_close.png') no-repeat;
 ...
 * @Date: 2024-05-28 17:22:57
 * @LastEditors: OctopusRoe
 * @LastEditTime: 2025-11-03 17:19:04
 * @Description:
 */
import { observable, makeObservable, action, toJS, makeAutoObservable, reaction } from 'mobx'
import request from '@/utils/request'
// import request from '../../service/request';
...
import {
  fullGradeList,
  middleBookVolumeList,
  middleSchoolBookPublisherList,
  middleSchoolSubjectList,
  primaryBookPublisherList,
  primaryBookVolumeList,
  primarySubjectList,
  seniorBookVolumeList,
  seniorSchoolBookPublisherList,
  seniorSubjectList,
} from '../modalContent/config'
import { getScreenHeader } from '@/utils/utilFunc'
import local from '@/utils/localTool'
...
export class IndexMobx {
  ...
  // eduGrade: string = '2';
  eduGrade: string = ''
  eduGradeList: any[] = fullGradeList
  // 学科
  eduSubject: string = ''
  eduSubjectList: any[] = primarySubjectList
  // 出版社
  // bookPublisher: string = '9';
  bookPublisher: string = ''
  bookPublisherList: any[] = primaryBookPublisherList
  // 册别
  bookVolume: string = ''
  bookVolumeList: any[] = primaryBookVolumeList

  // 轮播列表
  courseList: any[] = [
    {
      image: 'https://img0.baidu.com/it/u=3260383401,4157111732&fm=253&fmt=auto&app=138&f=GIF?w=889&h=500',
      name: '统编版我是中国人',
      grade: '一年级下册',
    },
    {
  ...
  refreshStorageData = async () => {
    const tvFilterData: any = await local.getItem('tvFilterData')
    console.log('%c Line:56 🍅 tvFilterData', 'color:#4fff4B', tvFilterData)
    if (tvFilterData) {
      const storageEduPhase = tvFilterData.eduPhase ? tvFilterData.eduPhase : '2'
      // 先赋值学段，让自动触发联动修改先执行
      this.setInfo({ eduPhase: storageEduPhase })
      // 手动触发联动修改
      this.eduPhaseRelateFilter(storageEduPhase, false, true)
      // 缓存数据最后赋值展示
      setTimeout(() => {
        this.setInfo({
          eduGrade: tvFilterData.eduGrade,
          eduSubject: tvFilterData.eduSubject,
          bookPublisher: tvFilterData.bookPublisher,
          bookVolume: tvFilterData.bookVolume,
        })
      }, 5)
    }
  }
  ...
  // 根据学段数据联动筛选展示项
  // needResetSelect是否需要重置选中项 needRefreshSubjectList是否需要联动学科列表
  eduPhaseRelateFilter = (
    eduPhase: string,
    needResetSelect: boolean = false,
    needRefreshSubjectList: boolean = true,
  ) => {
    if (eduPhase == '2') {
      this.setInfo({
        eduGradeList: fullGradeList.slice(0, 7),
        bookPublisherList: primaryBookPublisherList,
        bookVolumeList: primaryBookVolumeList,
      })
    } else if (eduPhase == '4') {
      this.setInfo({
        eduGradeList: fullGradeList.slice(7, 11),
        bookPublisherList: middleSchoolBookPublisherList,
        bookVolumeList: middleBookVolumeList,
      })
    } else if (eduPhase == '6') {
      this.setInfo({
        eduGradeList: fullGradeList.slice(11),
        bookPublisherList: seniorSchoolBookPublisherList,
        bookVolumeList: seniorBookVolumeList,
      })
    }
    if (needResetSelect) {
      this.setInfo({
  ...
  // 获取轮播列表
  getCarouselList = async (data: Object) => {
    const res: any = await request({
      url: `/smartScreen/api/resource/selectCourseResource`,
      method: 'POST',
      data: {
        eduPhase: this.eduPhase,
        eduGrade: this.eduGrade ? this.eduGrade : undefined,
        eduSubject: this.eduSubject ? this.eduSubject : undefined,
        bookPublisher: this.bookPublisher ? this.bookPublisher : undefined,
        bookVolume: this.bookVolume ? this.bookVolume : undefined,
        ...data,
      },
      headers: {
        ...getScreenHeader(),
    ...
    if (res && res.data && res.data?.records?.length) {
      console.log('%c Line:117 🌽 res.data', 'color:#fca650', res.data)
      this.setInfo({ courseList: res.data.records })
      // return res.data;
    } else {
      this.setInfo({ courseList: [] })
    }
  }
  ...
  resetMobx = () => {
    this.setInfo({
      ...
      eduGrade: '',
      eduGradeList: fullGradeList,

      eduSubject: '',
      eduSubjectList: primarySubjectList,
