...
<body>
  <div id="app">
    ...
    <h2
      class="zzz"
      :class="{mmm: true, 'lll': false, 'nnn': true}"
    >
      {{message}}
    </h2>
    <!-- 2.2动态切换 -->
...
<script>
  const app = Vue.createApp({
    data() {
      return {
        message: 'Hello Vue3',
        infos: { name: 'zzz', age: 23, height: 1.8 },
        name: 'zjf',
        age: 23,
        height: 1.8,
      }
...
<div id="app">
  ...
  <!-- 4.1对象写法 -->
  <div
    v-on="{ mousemove: onMouseMove, click: boxClick }"
    class="box"
  ></div>
  <div
    @="{ mousemove: onMouseMove, click: boxClick }"
    class="box"
  ></div>
</div>
  ...
  const app = Vue.createApp({
    data() {
      return {
        ...
        info: { name: 'zzz', age: 23, height: 1.8 },
      }
    },
  })
  app.mount('#app')
...
<template>
  <div class="app">
    ...
    <Counter
      :modelValue="appData"
      @update:modelValue="appData = $event"
    ></Counter>
    <Counter
      :model-value="appData"
      @update:model-value="appData = $event"
    ></Counter>

    <hr />
    <!-- 3.组件绑定多个v-model，修改modelValue为addD，所以可以绑定多个不同的v-model -->
    <Counter2
      v-model:appD="appData"
      v-model:hhh="text"
    ></Counter2>
  </div>
</template>
...
```vue
import { createApp } from 'vue' import App from './12_组件的混入/App.vue' import './global.css' const app =
createApp(App) app.mixin({ created() { console.log('全局混入，每个组件都有这部分逻辑') } }) app.mount('#app')
```
...
```vue
const formValue = reactive({ name: '张三', age: 18, address: { city: '北京' } });
```

```vue
1. 监听顶层属性 watch( () => formValue.name, // 只监听 name 的变化 (newName, oldName) => { console.log('name changed:',
newName, oldName); } );
```

```vue
2. 监听嵌套属性 watch( () => formValue.address?.city, (newCity, oldCity) => { console.log('city changed:', newCity,
oldCity); } );
```

```vue
3. 如果 formValue 是 ref const formValue = ref({ name: '张三', age: 18 }); // 监听 ref 内部对象的某个属性 watch( () =>
formValue.value.name, (newName, oldName) => { console.log(newName, oldName); } );
```

- 注意事项

* 如果监听的是对象或数组的某个属性，且该属性可能被整体替换（如 `formValue.address = {}`），那么监听其子属性可能无法捕获。这时可以改用监听整个 `address` 对象，并设置 `deep: true`，但这样也会捕获其内部所有变化。
* 如果你希望监听多个属性，可以同时监听多个 getter：

```vue
watch( [ () => formValue.name, () => formValue.age ], ([newName, newAge], [oldName, oldAge]) => { console.log('name or
age changed'); } );
```
...
<template>
  <div class="app">
    <HeaderContent
      :headerTitle="resData.title"
      :subTitle="resData.subtitle"
    ></HeaderContent>
    <div class="home_list">
      <template
        v-for="(item, index) of resData.list"
        :key="item.id"
      >
...
<script setup lang="ts">
  const props = defineProps<{
    levelFirstOptions: { value: string | number; label: string }[]
  }>()
  ...
  const getCampusList = async () => {
    try {
      const res = await getRegionListPage({
        pageNum: 1,
        size: 1000,
        dataKind: 1, // 假设1代表校区类型
      })
      if (res.data && (res.data as any).records) {
        options.value = (res.data as any).records
      }
    } catch (error) {
  ...
  watch(
    ...
    (newValue, oldValue) => {
      if (newValue) {
        formData.name = dayjs(toRaw(newValue)).format('YYYY') + '级'
      }
    },
    {
      immediate: false, // 初始化立即监听
      deep: false,
    },
  )
</script>
